...
:   사용자 프로그램의 함수와 함께 커널 함수를 추적한다.
    기본적으로는 추적하는 깊이는 1이다 (커널로의 진입 및 복귀 함수만 기록한다).
    이를 변경하려면 `-K`/`--kernel-depth` 옵션을 사용할 수 있다.
...
이 필터들은 런타임에 client에서 추가하거나 제거할 수 있다. agent operation에 대한 자세한 정보는 `uftrace-live`(1)을 참고한다.
...
Caller filter는 `@clear`을 붙이거나 `-C` / `--caller-filter` 옵션을 통해 런타임에 추가하거나 제거할 수 있다.
...
사용자는 네 개의 함수를 작성할 수 있다. 'uftrace_entry' 와 'uftrace_exit' 은
